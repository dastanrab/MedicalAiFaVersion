...
export function AdminUsersTableSkeleton({ rows = TABLE_ROWS }: { rows?: number }) {
    return (
        <>
            {Array.from({ length: rows }).map((_, i) => (
                <tr key={i} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-4 py-3">
                        <Skeleton className="mb-1.5 h-4 w-28 bg-slate-200/70" />
                        <Skeleton className="h-3 w-20 bg-slate-200/50" />
                    </td>
                    <td className="px-4 py-3">
                        <Skeleton className="h-6 w-24 rounded-lg bg-slate-200/60" />
                    </td>
                    <td className="px-4 py-3">
        ...
        <div className="space-y-6" aria-busy="true" aria-label="در حال بارگذاری کاربران">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                ...
                <table className="w-full">
                    <tbody>
                        <AdminUsersTableSkeleton rows={TABLE_ROWS} />
                    </tbody>
                </table>

...
import { sampleCounterparties } from '../data/sampleChats';
...
function mapApiToAdminChatRow(apiRoom: ApiChatRoom, index: number): AdminChatRow {
    return {
        id: index + 1,
        roomId: apiRoom.room_id,
        patientName: apiRoom.patient_name,
        patientPhone: apiRoom.phone || '—',
        counterpartyName: apiRoom.opponent_name,
        counterpartyType: apiRoom.role === 'doctor' ? 'doctor' : 'support',
        counterpartyId: apiRoom.opponent_id,
        lastMessage: apiRoom.last_message || '—',
        lastMessageAt: apiRoom.last_message_at,
        status: apiRoom.status === 1 ? 'open' : 'closed',
        flag: 'none',
        messageCount: 0,
        messages: [],
        province: undefined,
...
export function AdminChats() {
    ...
    const loadChatRooms = async (page: number) => {
        setApiLoading(true);
        setError(null);
        try {
            const response = await fetchChatRooms(page, pagination.perPage);
            const mappedChats = response.data.map((room, index) =>
                mapApiToAdminChatRow(room, index)
            );
            ...
            setError(error instanceof Error ? error.message : 'خطا در بارگذاری داده‌ها');
        } finally {
            setApiLoading(false);
    ...
    const handleRefresh = async () => {
        setLoading(true);
        try {
            await loadChatRooms(pagination.currentPage);
        } finally {
            setLoading(false);
        }
    };

    const updateChat = async (id: number, patch: Partial<AdminChatRow>) => {
        ...
        try {
            ...
            if (patch.status === 'closed') {
                await updateChatStatus(chat.roomId, 0);
            } else if (patch.flag === 'violation') {
                await markChatViolation(chat.roomId);
            } else if (patch.flag === 'referred') {
                await referToSupport(chat.roomId);
            }
            ...
            setError(error instanceof Error ? error.message : 'خطا در انجام عملیات');
        } finally {
    ...
    const referToSupportHandler = async (id: number) => {
        setActionLoadingId(id);
        setError(null);
        try {
            const chat = chats.find(c => c.id === id);
            ...
            console.error('Error referring to support:', error);
            setError(error instanceof Error ? error.message : 'خطا در ارجاع به پشتیبانی');
        } finally {
            setActionLoadingId(null);
        }
    };

    const loadChatDetails = async (chat: AdminChatRow) => {
        setError(null);
        try {
            const details = await fetchChatDetails(chat.roomId);

            const messages: AdminChatMessage[] = details.messages.map(msg => ({
                id: msg.id,
                sender: msg.sender === 'doctor' ? 'counterparty' : msg.sender,
                senderName: msg.sender_name,
                message: msg.message,
                sentAt: msg.sent_at,
                isSensitive: msg.is_sensitive,
            }));

            const updatedChat: AdminChatRow = {
                ...chat,
                messages,
                patientPhone: details.patient_phone || chat.patientPhone,
            ...
            setDetailsRow(updatedChat);
        } catch (error) {
            console.error('Error loading chat details:', error);
            setError(error instanceof Error ? error.message : 'خطا در بارگذاری جزئیات گفتگو');
            setDetailsRow(chat);
        }
    };
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div className="overflow-x-auto">
                    <table className="w-full text-right text-sm">
                        ...
                        <tbody>
                        {apiLoading ? (
                            <AdminChatsTableSkeleton rows={pagination.perPage} />
                        ) : paged.length === 0 ? (
                            <tr>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 px-4 py-3 text-xs text-slate-500">
                    <span>
                        نمایش {paged.length} از {filtered.length} گفتگو (کل: {pagination.total})
                    </span>
                    <div className="flex items-center gap-1">
                        <button
                            type="button"

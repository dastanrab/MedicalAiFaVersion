...
import { ArrowRight, Star, Loader2, User, Stethoscope, Send } from 'lucide-react';
...
export function DiagnosisResultV1() {
    ...
    useEffect(() => {
        if (requestPayload?.symptoms && isFirstRun.current) {
            ...
            sendMessage(requestPayload.symptoms);
        }
    }, [requestPayload]);
    ...
    const sendMessage = async (textContent: string) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'assistant', content: '⚠️ متأسفانه در ارتباط با سرور مشکلی پیش آمد.' }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        ...
        // آخرین پیام سیستم رو پیدا میکنیم
    ...
    return (
        <div className="flex flex-col h-dvh bg-gradient-to-b from-blue-50 to-white" dir="rtl">
            ...
            <div className="flex-1 overflow-y-auto w-full max-w-5xl mx-auto px-4 sm:px-6 pt-20 pb-4 flex flex-col">
                        ...
                        return (
                            <div key={idx} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`px-4 py-3 max-w-[85%] text-sm leading-relaxed shadow-sm ${
                                    ...
                                    {/* اگر پیام آخر است و وضعیت complete شده، تایپ نویسی نشان بده */}
                                    {isLastAndComplete ? (
                ...
                {/* ورودی چت برای زمانی که اطلاعات بیشتری نیاز است */}
                {status === 'chatting' && !loading && (
                ...
                {/* بخش نمایش نتایج نهایی (پزشکان، آزمایشگاه، فرم) */}

...
export const pharmacyAuthService = {
    async sendOtp(phone: string): Promise<void> {
        const res = await fetch(`${BASE_URL}/pharmacy/login`, {
            method: 'POST',
    ...
    async verifyOtp(phone: string, code: string): Promise<{ token: string; user: Record<string, unknown> }> {
        const res = await fetch(`${BASE_URL}/pharmacy/verify`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
    ...
    async getPharmacyProfile(token: string) {

...
interface ApiAiDiagnosis {
    session_id: string;
    created_at: string;
    diagnosis: AiDiagnosisDetail | null;
...
export default function DoctorAppointmentDetailPage() {
    ...
    const [aiDiagnoses, setAiDiagnoses] = useState<ApiAiDiagnosis[]>([]);
    ...
    const fetchAppointmentDetail = async () => {
        ...
        try {
            ...
            setAiDiagnoses(json.data.ai_diagnoses || []);
    ...
    const latestAiDiagnosis = aiDiagnoses.find(item => item.is_latest) || null;
    ...
    if (error || !appointment || !patient) {
        return (
            <div className="p-6">
                ...
                <Link
                    to="/provider/doctor/appointments"
                            ...
                            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                ...
                                >
                                    ...
                                    <span className="text-sm text-blue-600">
                                        {showAiChat ? 'بستن' : 'نمایش'}
                                    </span>
                                </button>

                                {!showAiChat && (
                                    <div className="mt-3 space-y-2 text-sm">
                                        {latestAiDiagnosis.diagnosis?.diagnosis &&
                                            latestAiDiagnosis.diagnosis.diagnosis.length > 0 && (
                                                <p>
                                                    <span className="text-slate-600">تشخیص:</span>{' '}
                                                    <span className="font-medium">
                                                        {latestAiDiagnosis.diagnosis.diagnosis.join('، ')}
                                                    </span>
                                                </p>
                                            )}

                                        {latestAiDiagnosis.urgency_level && (
                                            <p>
                                                <span className="text-slate-600">سطح فوریت:</span>{' '}
                                                <span className="font-medium">{latestAiDiagnosis.urgency_level}</span>
                                            </p>
                                        )}

                                        {latestAiDiagnosis.specialty?.primary && (
                                            <p>
                                                <span className="text-slate-600">تخصص:</span>{' '}
                                                <span className="font-medium">{latestAiDiagnosis.specialty.primary}</span>
                                            </p>
                                        )}

                                        {latestAiDiagnosis.diagnosis?.diagnosis_description && (
                                ...
                                {showAiChat && (
                                    <div className="mt-4 space-y-3 max-h-96 overflow-y-auto">
                                        {latestAiDiagnosis.messages.map((msg, idx) => (
                                            <div
                                                key={idx}
                                        ...
                                        {latestAiDiagnosis.notes && (
                                            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                                <p className="text-xs text-amber-700 mb-1">یادداشت نهایی</p>
                                                <p className="text-sm text-slate-800">{latestAiDiagnosis.notes}</p>
                                            </div>
                                        )}

                                        {latestAiDiagnosis.recommended_tests.length > 0 && (
                                            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                                                ...
                                                <ul className="text-sm text-slate-800 list-disc list-inside space-y-1">
                                                    {latestAiDiagnosis.recommended_tests.map((test, i) => (
                                                        <li key={i}>{test}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                ...
                                {previousAiDiagnoses.length > 0 && (
                                    <div className="mt-6 border-t border-blue-200 pt-4">
                                        <h3 className="text-sm font-semibold text-slate-800 mb-3">
                                            سوابق قبلی پیش‌تشخیص AI
                                        </h3>

                                        <div className="space-y-3">
                                            {previousAiDiagnoses.map((item, idx) => (
                                                <div
                                                ...
                                                >
                                                    ...
                                                    {item.diagnosis?.diagnosis &&
                                                        item.diagnosis.diagnosis.length > 0 && (
                                                            <p className="text-sm text-slate-700">
                                                                <span className="text-slate-500">تشخیص:</span>{' '}
                                                                <span className="font-medium">
                                                                    {item.diagnosis.diagnosis.join('، ')}
                                                                </span>
                                                            </p>
                                                        )}
                                                    ...
                                                    {item.urgency_level && (
                                                        <p className="text-sm text-slate-700 mt-1">
                                                            <span className="text-slate-500">سطح فوریت:</span>{' '}
                                                            <span className="font-medium">{item.urgency_level}</span>
                                                        </p>
                                                    )}

                                                    {item.specialty?.primary && (
                                                        <p className="text-sm text-slate-700 mt-1">
                                                            <span className="text-slate-500">تخصص:</span>{' '}
                                                            <span className="font-medium">{item.specialty.primary}</span>
                                                        </p>
                                                    )}

                                                    {item.diagnosis?.diagnosis_description && (
                                                    ...
                                                    {item.recommended_tests?.length > 0 && (
                                                        <div className="mt-2">
                                                            ...
                                                            <ul className="text-sm text-slate-700 list-disc list-inside">
                                                                {item.recommended_tests.map((test, i) => (
                                                                    <li key={i}>{test}</li>
                                                                ))}
                                                            </ul>
                                                        </div>
